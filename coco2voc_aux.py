...
def anns_to_seg(annotations, coco_instance):
    ...
    id_seg = np.zeros((h, w))
    masks, annotations = anns_to_mask(annotations, h, w)
...
def anns_to_mask(annotations, h, w):
    """
    ...
    """
    masks = []
    annotations = sorted(annotations, key=lambda x: x['area'])  # Smaller items first, so they are not covered by overlapping segs
